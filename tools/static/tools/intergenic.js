$(function () {
    ...
    $("#jstree_intergic").bind("loaded.jstree", function (event, data) {
        data.instance.open_all();
    });

    $('#jstree_intergic').jstree({
        "types": {
            "default": {
                "draggable": false
            },
            "strain": {
                "valid_children": [ "genome" ]
            },
            "genome": {
                'valid_children': [ 'none' ],
        ...
        "plugins": [ "wholerow", "checkbox", "types", "sort", "themes"]
    });
...
function getIntergenic() {
    var geneList = $('#geneList').val().replace(/(^\s*)|(\s*$)/g, '');  //remove space
    var genome = $("#jstree_intergic").jstree('get_selected');
    if (geneList.length + genome.length > 0) {
        openPostWindow('/tools/intergenicResult/', geneList,genome, '/search/intergenicResult/');
        //$.get("/tools/intergenicResult", {'geneList': geneList, 'genome': genome}, evalCallbk);  //get search types from server
    } else {
        //popover hint
        $('#intergenicAnalysis').popover({content: 'please input gene list or select genomes'});
        $('#intergenicAnalysis').popover('show');
        setTimeout(function () {
            $('#intergenicAnalysis').popover('destroy');
        }, 3000);
    }
...
function openPostWindow(url, data1,data2, name) {
    ...
    var hideInput = document.createElement("input");
    hideInput.type = "hidden";
    hideInput.name = "genome"
    hideInput.value = data2;
    tempForm.appendChild(hideInput);
...
function performFunction(item) {
    ...
    if (selectedNumber > 0) {
        ...
        if (functionType == "download") {  //download data
            ...
            $.fileDownload('/tools/intergenic/download/', {
                httpMethod: "POST",
                data: {
                    selected: selected,
